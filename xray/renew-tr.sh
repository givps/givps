...
# =========================================
# Name    : renew-tr
# Title   : Auto Script VPS - Renew Trojan Account
# Version : 1.2 (Revised)
...
return_to_menu() {
    if declare -f m-trojan >/dev/null 2>&1; then m-trojan; else exit 0; fi
}
...
if [[ ! -f "$USER_FILE" ]] || [[ ! -s "$USER_FILE" ]]; then
    ...
    echo -e "${red}=========================================${nc}"
    echo ""
    echo "  • ${yellow}You have no existing Trojan clients!${nc}"
    echo ""
    echo -e "${red}=========================================${nc}"
...
echo "  #    User        Expired"
echo -e "${red}=========================================${nc}"
# User file format: user password expired
mapfile -t USER_LIST < <(awk '{print $1}' "$USER_FILE")
awk '{printf "  %s.  %s\t%s\n", NR, $1, $3}' "$USER_FILE" | column -t
echo ""
...
# $3 is the expiration date in /etc/trojan/trojan-users
...
password=$(echo "$user_data" | awk '{print $2}')

# --- Update user file ---
echo -e "[ ${green}INFO${nc} ] Updating expiration date to $new_exp_date..."

# 1. Write the new user line to a temp file: User Password New_Expiry
echo "$selected_user $password $new_exp_date" > /tmp/trojan-new

# 2. Append all other users (excluding the renewed user)
grep -v "^$selected_user " "$USER_FILE" >> /tmp/trojan-new

# 3. Replace the original file
mv /tmp/trojan-new "$USER_FILE"

# Restart Xray (optional for Trojan, as credentials don't change, but fine for consistency)
systemctl restart xray >/dev/null 2>&1

# --- Display result ---
clear
echo -e "${red}=========================================${nc}"
echo -e "   ${green}Trojan Account Has Been Successfully Renewed!${nc}"
echo -e "${red}=========================================${nc}"
echo ""
