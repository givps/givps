...
# =========================================
# Name    : renew-ss
# Title   : Auto Script VPS - Renew Shadowsocks Account
# Version : 1.2 (Revised)
...
if [[ ! -f "$USER_FILE" ]] || [[ ! -s "$USER_FILE" ]]; then
    ...
    echo -e "${red}=========================================${nc}"
    echo ""
    echo "  • ${yellow}You have no existing Shadowsocks clients!${nc}"
    echo ""
    echo -e "${red}=========================================${nc}"
...
echo "  #    User        Expired"
echo -e "${red}=========================================${nc}"
# User file format: user password cipher expired
mapfile -t USER_LIST < <(awk '{print $1}' "$USER_FILE")
awk '{printf "  %s.  %s\t%s\n", NR, $1, $4}' "$USER_FILE" | column -t
echo ""
...
current_exp_date=$(awk -v usr="$selected_user" '$1 == usr {print $4; exit}' "$USER_FILE")
...
cipher=$(echo "$user_data" | awk '{print $3}')

# --- Update user file ---
echo -e "[ ${green}INFO${nc} ] Updating expiration date to $new_exp_date..."

# 1. Write the new user line to a temp file
echo "$selected_user $password $cipher $new_exp_date" > /tmp/ss-new
...
# Restart Xray (optional for SS, but good practice)
systemctl restart xray >/dev/null 2>&1

# --- Display result ---
clear
echo -e "${red}=========================================${nc}"
echo -e "   ${green}Shadowsocks Account Has Been Successfully Renewed!${nc}"
echo -e "${red}=========================================${nc}"
echo ""
