...
# =========================================
# Name    : del-tr
# Title   : Auto Script VPS - Delete Trojan Account
# Version : 1.2 (Revised)
...
if [[ ! -f "$USER_FILE" ]] || [[ ! -s "$USER_FILE" ]]; then
    ...
    echo -e "${red}=========================================${nc}"
    echo ""
    echo "  • ${yellow}No existing Trojan clients found!${nc}"
    echo ""
    echo -e "${red}=========================================${nc}"
...
# Note: $2 (password) is not needed for deletion logic, only for display/logging if desired.
...
if command -v jq &> /dev/null; then
    ...
    # The Xray config uses 'email' field for the username/tag in Trojan settings
