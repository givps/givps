...
# =========================================
# Name    : givps
# Title   : Auto Script VPS - Trojan Active User Check
...
if [[ ! -s "$USER_FILE" ]]; then
  echo -e "${yellow}No Trojan users found in $USER_FILE.${nc}"
  exit 0
fi

# 1) Get list of all known Trojan user emails
...
if [[ -f "$ACCESS_LOG" ]]; then
    awk -v OFS='|' -F'\t' '
        # Only process logs containing "trojan"
        /trojan/ {
...
if [[ $ACTIVE_FOUND -eq 0 ]]; then
    echo -e "${yellow}No Trojan users are currently connected.${nc}"
    echo -e "${red}=========================================${nc}"
else
    # 5) Display unmatched IPs
    sort -u "$TMP_MATCHED_IP" -o "$TMP_MATCHED_IP"
    echo -e "${blue}Other connected IPs (not matched to Trojan users):${nc}"
