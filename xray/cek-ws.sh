...
# =========================================
# Name    : check-vmess
# Title   : Auto Script VPS - VMess Active User Check
...
if [[ ! -s "$USER_FILE" ]]; then
  echo -e "${yellow}No VMess users found in $USER_FILE.${nc}"
  exit 0
fi

# 1) Get list of all known VMess user emails
...
if [[ -f "$ACCESS_LOG" ]]; then
    awk -v OFS='|' -F'\t' '
        # Only process logs containing "vmess"
        /vmess/ {
            # $4 contains the email (user)
            user = $4;
            # $5 contains the IP
            # Clean IP from port and brackets/IPv6 prefix
            ip = $5;
            gsub(/\r$/, "", ip);
            gsub(/:[0-9]+$/, "", ip);
            gsub(/\[|\]/, "", ip);
...
echo -e "${red}=========================================${nc}"
echo -e "${blue}           VMess Active Logins           ${nc}"
echo -e "${red}=========================================${nc}"
...
for u in "${USERS[@]}"; do
    ...
    if [[ -n "$ACTIVE_IPS" ]]; then
        ACTIVE_FOUND=1
        echo -e "${blue}User:${nc} ${yellow}$u${nc}"
        i=1
        
        while IFS= read -r ip; do
            # Get the latest hit and timestamp for this specific IP and user
            # Match the exact user and IP, and take the last record (which awk ensures is the latest based on its processing order)
...
if [[ $ACTIVE_FOUND -eq 0 ]]; then
    echo -e "${yellow}No VMess users are currently connected.${nc}"
    echo -e "${red}=========================================${nc}"
else
    # 5) Display unmatched IPs
    sort -u "$TMP_MATCHED_IP" -o "$TMP_MATCHED_IP"
    echo -e "${blue}Other connected IPs (not matched to VMess users):${nc}"
