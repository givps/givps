...
# Temporary files (auto removed on exit)
...
# 1) Collect users from config markers: ### username 2025-09-19 or #### username ...
mapfile -t USERS < <(grep -E '^#{3,4} ' "$CONFIG" 2>/dev/null | awk '{print $2}' | sort -u)

# 2) Collect connected remote IPs from xray established sessions
...
if [[ ${#USERS[@]} -eq 0 ]]; then
  echo -e "${yellow}No VMess users found in config markers (### or ####).${nc}"
fi

# 3) Loop through users: count hits and last seen per IP, show active ones
for u in "${USERS[@]}"; do
  [[ -z "$u" ]] && continue
...
# 4) Show "other" connected IPs not linked to any user
