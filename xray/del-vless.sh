...
# =========================================
# Name    : del-vless
# Title   : Auto Script VPS - Delete VLESS Account
# Version : 1.2 (Revised)
...
if [[ ! -f "$USER_FILE" ]] || [[ ! -s "$USER_FILE" ]]; then
    ...
    echo -e "${red}=========================================${nc}"
    echo ""
    echo "  • ${yellow}No existing VLESS clients found!${nc}"
    echo ""
    echo -e "${red}=========================================${nc}"
...
echo -e "${red}=========================================${nc}"
# Display format: #. User UUID Expired
...
# Note: UUID is not strictly needed for deletion but was extracted in the original script.
...
if command -v jq &> /dev/null; then
    ...
    # The Xray config uses 'email' field for the username/tag in VLESS settings
